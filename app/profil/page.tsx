...
function ProfilIcerik() {
  ...
  const getRutbeDetay = (r: string) => {
    switch (r) {
      case 'muhtar': return { label: 'MUHTAR', color: 'text-amber-400', border: 'border-amber-400/30', bg: 'bg-black/60', icon: <Trophy size={22} /> };
      ...
      <main className="flex-1 lg:ml-80 relative bg-white/30 backdrop-blur-sm pb-20 z-10">
        ...
        <div className="max-w-6xl mx-auto px-10 pt-12 space-y-10">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <section className="bg-black text-white p-8 rounded-[3rem] border border-white/10 shadow-2xl relative overflow-hidden">
          ...
          <section className={`p-12 rounded-[4rem] border shadow-2xl relative overflow-hidden transition-all duration-700 backdrop-blur-2xl ${statu === 'muhtar' ? 'bg-black/90 text-white border-amber-400/50' : 'bg-white text-black border-slate-200'}`}>
            <div className="flex flex-col md:flex-row gap-12 items-center">
              <div className={`w-48 h-48 rounded-[3.5rem] flex items-center justify-center border-[8px] shadow-2xl text-6xl font-black ${statu === 'muhtar' ? 'bg-amber-400 text-black border-white/10' : 'bg-black text-white border-white'}`}>
                {initials}
              </div>
              ...
              {radarBinalar.length > 0 ? radarBinalar.map((bina) => (
                <div key={bina.id} className="bg-white/60 backdrop-blur-2xl p-8 rounded-[3.5rem] border border-white shadow-xl group hover:border-blue-600 text-black transition-all">
                  <div className="flex justify-between items-start mb-6">
                    <div className="w-12 h-12 bg-blue-50/50 rounded-2xl flex items-center justify-center text-blue-600"><MapPin size={22} /></div>
                    <button onClick={() => radarSil(bina.id)} className="p-3 bg-black/5 text-slate-300 rounded-xl hover:text-red-500 hover:bg-red-50 transition-all"><X size={18} /></button>
                  </div>
