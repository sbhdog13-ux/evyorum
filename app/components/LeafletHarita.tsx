...
export default function LeafletHarita({ binalar = [], ilcePuanlari = {}, legend = false }: { binalar?: { ad: string; koordinat: string; finalPuan: number; sayi: number }[]; ilcePuanlari?: { [ilce: string]: { toplam: number; sayi: number } }; legend?: boolean }) {
    ...
    const init = () => {
      ...
      binalar.forEach(b => {
        ...
        L.marker([lat, lng], { icon }).addTo(map)
          .on('click', () => setSecili({ ad: b.ad, lat, lng, finalPuan: b.finalPuan, sayi: b.sayi, slug: slugify(b.ad) }));
      });
    };
  ...
  return (
    <div className="w-full h-full relative">
      ...
      {/* Binaya dokununca: sokak görünümü + karneye git */}
      {secili && (
        <div className="absolute inset-x-0 bottom-0 z-[1100] bg-white rounded-t-[2rem] shadow-2xl overflow-hidden border-t border-slate-200">
          <div className="px-5 py-3 bg-[#023E56] text-white flex items-center justify-between">
            <div className="font-black italic uppercase text-[14px] truncate pr-2">{secili.ad}</div>
            <button onClick={() => setSecili(null)} className="text-white/70 hover:text-white text-2xl leading-none shrink-0">×</button>
          </div>
          <div className="relative bg-black">
            ...
            <SokakGorunumu lat={secili.lat} lng={secili.lng} className="w-full" style={{ height: '30vh', minHeight: 160, maxHeight: 260 }} />
          </div>
          <div className="p-4 flex items-center gap-3">
            <div className="flex items-center gap-1.5 bg-slate-50 rounded-2xl px-4 py-2.5"><span className="text-[20px] font-black italic" style={{ color: secili.finalPuan >= 4 ? '#16a34a' : secili.finalPuan >= 2.5 ? '#eab308' : '#dc2626' }}>{secili.finalPuan.toFixed(1)}</span><span className="text-yellow-400 text-[15px]">★</span></div>
            <div className="text-[11px] font-bold text-slate-400 uppercase">{Math.round(secili.sayi)} mühür</div>
            <a href={`/bina/${secili.slug}`} className="ml-auto bg-blue-600 text-white font-black italic uppercase text-[12px] px-5 py-3 rounded-2xl hover:bg-[#023E56] transition-colors">Karneyi Gör →</a>
          </div>
        </div>
      )}
    </div>
