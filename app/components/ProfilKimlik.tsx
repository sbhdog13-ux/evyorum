...
const PRESETLER = [
  { id: 'mavi', bg: '#2563eb', sekil: 'kare', etiket: 'profil.av1' },
  { id: 'siyah', bg: '#000000', sekil: 'kare', etiket: 'profil.av2' },
  { id: 'mor', bg: '#7c3aed', sekil: 'yuvarlak', etiket: 'profil.av3' },
  { id: 'yesil', bg: '#059669', sekil: 'kare', etiket: 'profil.av4' },
  { id: 'turuncu', bg: '#ea580c', sekil: 'yuvarlak', etiket: 'profil.av5' },
];
...
const DESENLER: { id: 'dolu' | 'cizgili' | 'noktali'; etiket: string }[] = [
  { id: 'dolu', etiket: 'profil.desen1' }, { id: 'cizgili', etiket: 'profil.desen2' }, { id: 'noktali', etiket: 'profil.desen3' },
];
    ...
    <>
      <div className="flex items-center gap-5">
        ...
        <button onClick={() => { setYeniIsim(displayName); setModal('isim'); }} className="text-left" title="İsmi düzenle">
          ...
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1.5">{t('profil.isimDuzenle')}</div>
        </button>
      </div>
      ...
      {modal !== 'yok' && (
        <div className="fixed inset-0 z-[600] bg-black/50 flex items-center justify-center p-6" onClick={() => setModal('yok')}>
          <div className="bg-white rounded-[2rem] p-8 w-full max-w-md" onClick={e => e.stopPropagation()}>
            {modal === 'isim' ? (
              <>
                <h3 className="font-black uppercase italic text-[16px] mb-5">{t('profil.isimGuncelle')}</h3>
                ...
                <div className="flex gap-3 mt-5">
                  <button onClick={() => setModal('yok')} className="flex-1 py-3.5 rounded-2xl bg-slate-100 font-black uppercase italic text-[12px] text-slate-500">{t('profil.iptal')}</button>
                  <button onClick={isimKaydet} className="flex-1 py-3.5 rounded-2xl bg-blue-600 text-white font-black uppercase italic text-[12px]">{t('profil.kaydet')}</button>
                </div>
              </>
            ) : (
              <>
                <h3 className="font-black uppercase italic text-[16px] mb-5 text-center">{t('profil.avatarSec')}</h3>
                <div className="flex justify-center mb-5">
                  <AvatarGoster secim={tab === 'preset' ? gecici : { tip: 'ozel', renk: ozelRenk, desen: ozelDesen }} harf={harf} size={84} />
                </div>
                <div className="flex gap-2 mb-5">
                  <button onClick={() => setTab('preset')} className={`flex-1 py-2.5 rounded-xl text-[11px] font-black uppercase italic border-2 ${tab === 'preset' ? 'border-blue-600 text-blue-600 bg-blue-50' : 'border-slate-100 text-slate-400'}`}>{t('profil.hazir')}</button>
                  <button onClick={() => setTab('ozel')} className={`flex-1 py-2.5 rounded-xl text-[11px] font-black uppercase italic border-2 ${tab === 'ozel' ? 'border-blue-600 text-blue-600 bg-blue-50' : 'border-slate-100 text-slate-400'}`}>{t('profil.kendinYarat')}</button>
                </div>
                {tab === 'preset' ? (
                  <div className="grid grid-cols-5 gap-3">
                    {PRESETLER.map(p => (
                      <button key={p.id} onClick={() => setGecici({ tip: 'preset', id: p.id })}
                        ...
                        <span className="text-[8px] font-black text-slate-400 uppercase">{t(p.etiket)}</span>
                      </button>
                    ))}
                  </div>
                ) : (
                  <>
                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('profil.renkSec')}</div>
                    ...
                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('profil.desenSec')}</div>
                    <div className="flex gap-2">
                      {DESENLER.map(d => (
                        <button key={d.id} onClick={() => setOzelDesen(d.id)}
                          className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase italic border-2 ${ozelDesen === d.id ? 'border-blue-600 text-blue-600 bg-blue-50' : 'border-slate-100 text-slate-400'}`}>{t(d.etiket)}</button>
                      ))}
                    </div>
                  </>
                )}
                <div className="flex gap-3 mt-6">
                  <button onClick={() => setModal('yok')} className="flex-1 py-3.5 rounded-2xl bg-slate-100 font-black uppercase italic text-[12px] text-slate-500">{t('profil.iptal')}</button>
                  <button onClick={avatarKaydet} className="flex-1 py-3.5 rounded-2xl bg-blue-600 text-white font-black uppercase italic text-[12px]">{t('profil.kaydet')}</button>
                </div>
              </>
            )}
          </div>
